...
static void set_mode(uint8_t mode)
{
	...
	case C_TAKEOFF:	// 13:CHASERモード用単独テイクオフ
        ap.manual_attitude = false;
    	ap.manual_throttle = false;
        do_c_takeoff();
		break;
	
	case CHASER:		// 14:CHASERモード
		ap.manual_attitude = false;
    	ap.manual_throttle = false;
		set_yaw_mode(YAW_CHASER);		// 10:YAW_CHASERモード
		set_roll_pitch_mode(ROLL_PITCH_AUTO);		// ロールピッチAUTO
		set_throttle_mode(THROTTLE_AUTO);		// スロットルAUTO。ALTは目標高さに、みたいに書いてあった
		set_nav_mode(NAV_CHASER);		// 4:CHASERモード
		break;
